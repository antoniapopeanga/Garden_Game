...
public class MapLevel {
    ...
    public static MapEntity[][] parse(String text) {
        ...
        MapLevel mapLevel = new MapLevel(width, height); // Create an instance of MapLevel

        for (int j = 0; j < height; j++) {
            String line = lines[j];
            for (int i = 0; i < width; i++) {
                mapLevel.set(i, j, MapEntity.fromCode(line.charAt(i))); // Call set method on the mapLevel instance
            }
        }

        return mapLevel.grid;
    }
